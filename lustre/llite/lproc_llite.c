...
static ssize_t hybrid_io_hdd_write_threshold_bytes_show(struct kobject *kobj,
							struct attribute *attr,
							char *buf)
{
	struct ll_sb_info *sbi = container_of(kobj, struct ll_sb_info,
...
static ssize_t hybrid_io_hdd_write_threshold_bytes_store(struct kobject *kobj,
							 struct attribute *attr,
							 const char *buffer,
							 size_t count)
{
	...
	sbi->ll_hybrid_io_hdd_write_threshold_bytes = val;

	return count;
}
LUSTRE_RW_ATTR(hybrid_io_hdd_write_threshold_bytes);

static ssize_t hybrid_io_hdd_read_threshold_bytes_show(struct kobject *kobj,
						       struct attribute *attr,
						       char *buf)
{
	struct ll_sb_info *sbi = container_of(kobj, struct ll_sb_info,
...
static ssize_t hybrid_io_hdd_read_threshold_bytes_store(struct kobject *kobj,
							struct attribute *attr,
							const char *buffer,
							size_t count)
{
	...
	sbi->ll_hybrid_io_hdd_read_threshold_bytes = val;

	return count;
}
LUSTRE_RW_ATTR(hybrid_io_hdd_read_threshold_bytes);
...
static struct attribute *llite_attrs[] = {
	...
	&lustre_attr_hybrid_io_hdd_read_threshold_bytes.attr,
