...
static inline void osd_trans_commit_cb(struct osd_thandle *oh, journal_t *journal,
...
static int cmp_key_txn(const void *transaction, const struct rb_node *node)
{
	...
	oh = container_of(node, struct osd_thandle, ot_node);
	if (transaction > (void *)oh->ot_transaction)
		return 1;
	else if (transaction < (void *)oh->ot_transaction)
		return -1;
	return 0;
}
